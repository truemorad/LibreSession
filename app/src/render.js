...
const minMultSwitch = (object, array) => {
  if (object == 'mult') {
    return array[0];
  }
  if (object == 'max') {
...
maxBtn.addEventListener('click', async () => {
  ...
  maxBtn.innerHTML = minMultSwitch(result, minMultUI);
});
...
optionsBtn.addEventListener('click', () => {
  ...
  const colorSwitcher = async (element) => {
    ...
    const value = document.documentElement.getAttribute('style');
    const result = await window.electronAPI.storeUserData(value);
    console.log(result);
  };
